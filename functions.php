...
function mannering_music_filter_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		/* translators: %s: $title */
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'mannering_music' ), max( $paged, $page ) );
	}
...
if ( ! function_exists( 'mannering_music_setup' ) ) :
	...
	function mannering_music_setup() {
		...
		register_nav_menus(
			array(
				'main'      => esc_html__( 'Main', 'mannering_music' ),
				'secondary' => esc_html__( 'Secondary', 'mannering_music' ),
